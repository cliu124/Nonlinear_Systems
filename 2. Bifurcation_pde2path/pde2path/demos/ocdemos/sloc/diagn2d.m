function dia=diagn2d(p,wnr) 
s1=p.oc.s1; u0=p.u0; u1=p.oc.u1; 
sol=p.cp; T=sol.par(1); 
t=T*sol.t; y=sol.u; tl=length(t); np=s1.nu/2; dia=zeros(2,tl); 
for i=1:tl
...
return 
%plot(t,jcav(:),'k',t,jcavd(:),'b','LineWidth',2); legend('J_{c,a}','e^{-\rho t}J_{c,a}');  
plot(t,jcav(:),'k','LineWidth',2); 
xlabel('t'); fprintf('J0=%g, Jp=%g, J1=%g\n',j0,jp,j1); 
tit={['J=' mat2str(real(jp),4) ',  J_1=' mat2str(j1,4)]}; 
figure(wnr); set(gca,'FontSize',14); title(tit); axis tight;
