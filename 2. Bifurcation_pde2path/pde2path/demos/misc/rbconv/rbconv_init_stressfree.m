function p=rbconv_init_stressfree(p) 
...
p.fuha.bc=@rbconv_bc_stressfree; p.fuha.bcjac=@rbconv_bc_stressfree; 

% domain and mesh
lx=2; lz=0.5; p.mesh.geo=rec(lx,lz); p=stanmesh(p,100,25); p=setbmesh(p);
p.tau=zeros(p.nc.neq*p.np+1,1); 

% continuation settings
p.nc.ilam=1;
p.sol.xi=1/p.np; p.sol.ds=0.1; p.nc.dsmin=0.00001; p.nc.dlammax=50; p.nc.nsteps=5; p.nc.neig=20; 
