...
% rmdegfaces: mod of REMOVE_DEGENERATE_FACES from gptoolbox 
  max_iter=100; epsilon=0; keepbd=0; areafac=2; 
  params_to_variables=containers.Map( ...
    {'MaxIter','Epsilon','keepbd','areafac'}, {'max_iter','epsilon','keepbd','areafac'}); %HU 
  v=1;
 ...
 % if keepbd; D(bdtrili(FF))=0; end 
  ...
  while true
    ...
    [BDF,bdJ,K]=boundary_faces(FF); 
    D=A/areafac < epsilon; %D=A/2 < epsilon; AM
    if keepbd; D(bdtrili(FF))=0; end 
