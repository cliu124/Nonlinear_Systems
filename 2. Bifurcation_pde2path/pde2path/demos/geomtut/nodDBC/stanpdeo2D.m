...
methods(Access = public)
  function o=stanpdeo2D(lx,ly,varargin) % constructor 
     ...
     if nargin==3; o.grid.square(-lx,lx,-ly,ly,varargin{1}); end 
     if nargin==4; % lx,ly, nx, ny; or lx, ly, h, sw;
        if ~isstruct(varargin{2}); o.grid.square(-lx,lx,-ly,ly,varargin{1},varargin{2});
        else hmax=varargin{1}; sw=varargin{2}; 
            try sym=sw.sym; catch; end 
            switch sym; % 
                case 0; o.grid.square(-lx,lx,-ly,ly,hmax); 
                case 1; o.grid.ccsquare(-lx,lx,-ly,ly,hmax); 
                case 2;  % true criss-cross via sympoi
                    o.grid.square(0,lx,-ly,ly,hmax); 
     ...
     if nargin==5;  % lx,ly, nx, ny, sw. 
        ...
        try sym=sw.sym; catch; end
         switch sym; % 
            case 0; o.grid.squarechebx(-lx,lx,-ly,ly,nx,ny); 
                %o.grid.square(0,lx,-ly,ly,nx,ny);
            case 1; o.grid.ccsquare(-lx,lx,-ly,ly,nx,ny); 
            case 2; p.plot.ifig=6; p.nc.neq=1; % fill p with some dummys  
                o.grid.squarechebx(-lx,lx,-ly,ly,nx,ny);  
                %o.grid.square(-lx,lx,-ly,ly,nx,ny); 
                rlongs=o.grid.rlong;  o.grid.rlong=1;
                idx=1:size(o.grid.t,2); % to refine all 
                 o.grid.refineMesh(idx); o.grid.rlong=rlongs;                
         end
