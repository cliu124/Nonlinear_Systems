...
% singular M=diag(M,0) mass-matrix to compute correct eigenvalues, here with pBC 
p=stanparam(p); p.nc.neq=2; p.ndim=ndim; p.nc.neig=40; 
p.fuha.sG=@sG; p.fuha.sGjac=@sGjac; p.fuha.qf=@qf; p.fuha.qfder=@qfder; p.nc.nq=0; 
...
ny=round(nx*ly/lx); try sw=varargin{1}; catch; sw=[]; end; 
...
p.nc.lammin=-4; p.nc.lammax=1.1; p.nc.ds=0.01; p.nc.dsmax=0.1; 
u=0*ones(p.np,1); v=u; u0=[u v]; p.u=u0(:); 
p.u=[p.u; par]; p.nc.ilam=1; p.file.smod=5; 
p=box2per(p,[1 2]); % switching on pBC in x and y
p=setfemops(p); p.nc.nsteps=100; p.usrlam=[0 1]; 