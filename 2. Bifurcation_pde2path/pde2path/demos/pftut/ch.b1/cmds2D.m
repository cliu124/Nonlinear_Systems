...
p=seltau(p0,3,'2D1-st',3); p.sol.ds=0.1; p=cont(p,60); % stripes
%% 2ndary bifs 
...
figure(2); clf; p=spcontini('2D1-sp','fpt2',2,'fp2c'); p.sol.ds=-0.001; plotsol(p); 
p.sw.bifcheck=0; p.nc.dsmax=0.05; p.file.smod=5; p.usrlam=[0.05 0.02]; 
p.nc.del=1e-4; p.nc.tol=1e-6; p.sw.spjac=1; p.sw.spqjac=1; p.fuha.spqjac=@stanspqjac; 
huclean(p); p=cont(p,30); 
%% BD for fold-cont 
figure(3); clf; plotbra('fp2c','pt30',3,1,'lab',[0 20 40]); xlabel('\epsilon'); 
%% soln plots 
mypsol('fp2c','pt0'); mypsol('fp2c','pt30');
%% FP cont exit 
huclean(p); p=spcontexit('fp2c','pt30','sp2a'); p.sw.spcalc=1; p.usrlam=0; 
p.nc.dsmax=0.2; p.nc.lammax=1; p.nc.lammin=-1; p.plot.bpcmp=5;  p=cont(p,40); 
%% BD at small eps 
