...
if ischar(varargin{1})
    ...
    if exist(str,'file')==2
        ...
        if (pt(1)=='p' && pt(2)=='t') || (pt(1)=='f' && pt(2)=='p' && pt(3)=='t') ...
                || (pt(1)=='b' && pt(2)=='p' && pt(3)=='t')
...
for k=1:nargin
 if ischar(varargin{k})
   switch lower(varargin{k})
     ...
     case 'sh'; shsw=varargin{k+1}; 
   end
 end
end
if pstyle==-1; userplot(p,wnr); return; end 
% set fig. nr. and u
figure(wnr); try upde=p.mat.fill*p.u(1:p.nu); catch; upde=p.u; end 
...
switch size(po,1)
  ...
  % plotsol(p,11,[1,2],'lw',{'b3','b3'}), we fill p.plot.lw
    ...
    set(gca,'FontSize',p.plot.fs); title([tname],'interpreter','tex');
...
if pstyle==1; figure(wnr); slplot(p,p.plot.ng,u,p.plot.fs);  % p.pdeo.grid.plotSlices(u); 
    try colormap(p.plot.cm); catch, colormap cool;  end;  
    axis image;  colorbar('southoutside');  
end
if pstyle==3; figure(wnr); clf; faceplot(p,u); % p.pdeo.grid.plotFaces(u,'LineStyle','none'); 
    try colormap(p.plot.cm); catch, colormap cool; end;      
...
if pstyle==4; try cut=p.plot.cut; catch; cut=[0 -1e6 -1e6]; end 
    figure(wnr); clf; p.pdeo.grid.cutplot(u,cut); 
...
if size(po,1)==3 && pstyle==2
  title({t1,t},'fontsize',14); %p.plot.fs); % write point name and levels
else
    title(t1,'fontsize',14); % write point name
end
end
