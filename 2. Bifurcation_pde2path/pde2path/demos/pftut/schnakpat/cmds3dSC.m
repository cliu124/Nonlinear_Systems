...
p=[]; kc=sqrt(sqrt(2)-1); lx=pi/kc; ly=lx; lz=lx; par=[3.24, 0, 60];
nx=4; sw.sym=1; sw.ref=2; dir='3DSC'; % np=9800, still OK
p=schnakinit(p,[lx,ly,lz],nx,par,sw); p.pdeo.grid.plotFaces; p.np, pause 
p.pm.resfac=1e-4; p.sol.ds=-0.1; p=setfn(p,dir); p.sw.verb=2; p.nc.bisecmax=4; 
p.nc.neig=4; p.nc.eigref=-0.5; p=cont(p,3); p.sw.bifcheck=0; p=cont(p,10); 
...
p0=cswibra('3DSC','bpt1',aux); p0.nc.dsmin=0.01; p0.sw.bifcheck=0; p0.pm.resfac=1e-6; p0.pm.mst=10; 
p0.file.smod=5; p0.nc.tol=1e-8; p0.nc.lammin=3; p0.sol.ds=0.01; p0.sw.foldcheck=0; 
