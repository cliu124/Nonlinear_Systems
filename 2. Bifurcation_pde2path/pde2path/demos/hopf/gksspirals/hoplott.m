...
[m,x0i]=min(x.^2+y.^2); [m,x1i]=min((x-1).^2+y.^2);% x0i, x(x0i),y(x0i), x1i, x(x1i), y(x1i), pause 
n=p.nu/p.nc.neq; 
switch ndim
...
figure(6); clf; plot(T*tv, z(x0i,:),'-k', 'linewidth',2); hold on; 
plot(T*tv, z(x1i,:),'-b','linewidth',2); hold off; 
legend('u_1((0,0),t)','u_1((1,0),t)'); xlabel('t'); 
axis tight; 
...
if isfield(p,'pdeo'); cmp=1; p.pdeo.grid.plot(u((cmp-1)*p.np+1:cmp*p.np),'LineStyle','none'); colorbar off; 
    if isfield(aux,'cax'); axis(aux.cax); axset=1; end  % computed (uniform) axes 
    hold on; 
    xt=gettip(p,u,[0.2 -0.2],0.001); plot3(xt(1),xt(2),2,'*k');  
    hold off; 
else 
    ...
    if pstyle==2; pdeplot(po,ed,tr,'xydata',u(1:p.np)); box on; colorbar off; 
        if isfield(aux,'cb'); if aux.cb==1; colorbar; end; end; end 
    if pstyle==3; h=pdesurf(po,tr,u(1:p.np)); view(10,40); colorbar off; 
...
if isfield(aux, 'view'); view(aux.view); end 
end
