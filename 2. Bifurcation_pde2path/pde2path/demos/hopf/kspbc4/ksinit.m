function p=ksinit(p,nx,lx,par) % 4th order form
p=stanparam(p); p.nc.neq=1; p.ndim=1; 
...
p.nc.lammin=1e-4; p.sw.spcalc=1; p.sw.bifcheck=2; p.sw.verb=2; 
u=par(2)*ones(p.np,1); 
p.u=[u; par]; p.file.smod=0; p.sol.ds=-0.01; p.nc.dsmax=0.02;
p=box2per(p,1); 
