...
if noa==1; % enables the querys for varargin{2} if no second input is given.
    varargin{2}={'no options'};
end
p=varargin{1}; % check if varargin{1} is struc or dir and load p if necessary
if ischar(p); % call like plotbra('h',...)
    pt=char(['pt' mat2str(max(getlabs(p)))]);
   ...
   try p=loadp(p,pt); catch
       warning(['Point ' p '/' pt ' does not exist. Plotting whole branch instead.']);
       pt=char(['pt' mat2str(max(getlabs(p)))]);  p=loadp(p,pt);
   end
end
...
for i=fp:lp-1 % loop from first-point to last point
    ...
    % initial point (-1) (also from swibra (-2)) or branch point (1). In
    % this case line width is from second point.
    lw=lwun; lt=tyun; 
...
for i=fp:lp; % plot markers for branch/fold/hopf points
    ...
    if(ms>0 && ismember(p.branch(2,i),3)) % plot markers for hopf points
      plot(p.branch(xcmp,i),p.branch(length(bradat(p))+cmp,i),'d','MarkerSize',ms*lwun,'color',cl); 
    end
    if(fms>0 && ismember(p.branch(2,i),2)) % plot markers for fold points
      plot(p.branch(xcmp,i),p.branch(length(bradat(p))+cmp,i),'x','MarkerSize',fms*lwun,'color',cl); 
    end
end
