classdef plotUtils3D < handle
    % plotUtils3D - collects common methods for 
    % plotting 3D meshes and objects .
    % Provides:
    %     plotFaces
    %     plotIso
    %     cutawayPlot
    ...
    % Changes: 10/2014
    % Rename identifyBoundary to identifyBoundarySegment
    % Empty call of identifyBoundarySegment plot now all segments in
    % subplots. 
    % Changes 3.2015
    % plotIso can now handle plot options.
    % Also a set of levels  can be plotted
    % system.grid.plotIso(system.u(:,end),[.1 1 3],'SpecularColorReflectance',0.5);
    % plot levels 0.1 1 and 3, and also sets the color reflection to 0.5,
    % See Matlab help for options.
    % Correct some empty argument calls
...
% use the plotFaces
    ...
    methods(Static,Access=protected,Hidden)    
        function un=p3interpol(xn,yn,zn,u,x,y,z)
            ...
            F=scatteredInterpolant(xv,yv,zv,uv);
            un=F(xn,yn,zn);
        end
    end
end