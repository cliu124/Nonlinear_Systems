...
%      nPointsInElements double = 2;
end

properties(Constant = true)
    spaceDimension double = 1;
end
methods(Access = public)       
 ...
 obj.p=linspace(geo(1),geo(2),nx); % (1 mesh-ref somehow needed ...)
    ...
    function [cval,aval,fval] = aCoefficientsMpt(obj,c,a,f)
        n = obj.nElements; x = obj.point2Center(obj.p);
        if isa(c,'function_handle') || isa(c,'inline'); cval = feval(c,x);
        elseif isa(c,'char'); cval = eval(c).*ones(1,n);
        elseif isa(c,'numeric')
            if length(c)==n;  cval = c(:)'; % f vector               
            elseif length(c)==obj.nPoints;  cval = obj.point2Center(c);
            elseif length(c)==1; cval = c*ones(1,n); % scalar                
            else error('wrong sized c')
            end
        elseif isa(c,'inline'); cval = c(p);
        else; error('wrong formated c')
        end

        if isa(a,'function_handle') || isa(a,'inline'); aval = feval(a,p);
        elseif isa(a,'char'); aval = eval(a).*ones(1,n);            
        elseif isa(a,'numeric')
            if length(a)==n;  aval = a; aval = aval(:)'; % f vector
            elseif length(a)==obj.nPoints; aval = obj.point2Center(a);
            elseif length(a)==1;  aval = a*ones(1,n);% skalar
            else; error('wrong sized a')
            end
        elseif isa(a,'inline'); aval = a(p);
        else; error('wrong formated a')
        end

        if isa(f,'function_handle') || isa(f,'inline'); fval = feval(f,x);
        elseif isa(f,'char'); fval = eval(f).*ones(1,n);
        elseif isa(f,'numeric')
            if length(f)==n;  fval = f; % f vektor
            elseif length(f)==obj.nPoints; fval = obj.point2Center(f);     
            elseif length(f)==1; fval = f*ones(1,n);    % skalar
            else; error('wrong sized f')
            end
        elseif isa(f,'inline'); fval = f(p);
        else; error('wrong formated f')
        end
    end
