classdef gridd < handle
    ...
    properties(Access=public)
        ...
        %         g % geometry   not in pde free version     
        p %@double % points
        e %@double % edges 
        t %@double % triangles
        rlong=0; % HU 
    
        nPointsInElements %@double % Number of points in element,...
                                 % two in 2D, three or four in 2D,...
                                 % four or six in 3D
                         
        isExtended %@logical = false  % if expanded then e.g. 
    ...
    E = sparse(1:obj.nPoints,1:obj.nPoints,1./sum(E,2))*E;            
        ...
        function obj2 = copy(obj1)
            % abstract hardcopy method            
            obj2 = eval(class(obj1));            
            propNames = properties(obj1);          
            for k = 1:length(propNames)
                try
                    eval(['obj2.',propNames{k},' = obj1.',propNames{k},';']);
                catch
                    % here we catch al private props that cannot copied
                end
            end            
        end
