function [x,p]=lssILUb(A,b,p)
% lssILUb: Uses ilupack4m (https://github.com/fastsolve/ilupack4m) for solving Ax=b. 
% 
% [x,p]=lssILUb(A,b,p)
% see also: lss, lssbel, blssbel, belpi, bel
nb=norm(b(:),inf); if nb==0; x=b; return; end   % rhs=0 
if ~isfield(p,'amgopt'); p.amgopt=ILUinit(A,[]); end % very first call of lssILU 
    ...
    if used_it>p.amgopt.maxit-1 % if r is still too large or prec already new 
       ...
       switch choi; 
          ...
          if p.ilup.droptol>10*(p.ilup.droptolmin-1e-10)
            ...
            p.mat.prec=ILUdelete(p.mat.prec); p.mat=rmfield(p.mat,'prec'); % clean mem  
            error(['ILUsolver did no converge with droptol=' mat2str(p.amgopt.droptol) '. Stopping.']);  
          end
          ...
          if p.amgopt.maxit<(p.ilup.maxitmax+1)/2; 
            ...
            t1=tic; [x,p.amgopt]=ILUsolver(A,p.mat.prec,p.amgopt,b); t1=toc(t1); 
            fprintf('ILUsolver in %g seconds, niter=%i \n', t1,used_it);
          else % clean mem and stop 
            p.mat.prec=ILUdelete(p.mat.prec); %p.mat=rmfield(p.mat,'prec'); % clean mem  
            error(['ILUsolver did no converge with maxit=' mat2str(p.amgopt.maxit) '. Stopping.']); 
          end
       end
    end
end