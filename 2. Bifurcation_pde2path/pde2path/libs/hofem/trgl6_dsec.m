function [ne,ng,p,c,efl,gfl]=trgl6_dsec(ndiv,r,phi)
% trgl6_dsec: triangule disk-sector into 6-node elements, afteer FSElib 
% by the successive subdivision of two parental elements
act=r;  % disk radius
% parental structure with two elements
ne=2; xf=cos(phi); yf=sin(phi); 
x(1,1)=0.0; y(1,1)=0.0; efl(1,1)=0; % first element
x(1,2)=act; y(1,2)=0.0; efl(1,2)=1;
x(1,3)=xf*act; y(1,3)=yf*act; efl(1,3)=1;
x(2,1)=0.0; y(2,1)=0.0; efl(2,1)=0; % second element
x(2,2)=act; y(2,2)=0; efl(2,2)=1;
x(2,3)=xf*act; y(2,3)=-yf*act; efl(2,3)=1;

for i=1:ne
 efl(i,4)=0; efl(i,5)=1; efl(i,6)=0;
end

% element midpoints
for i=1:ne
 x(i,4)=0.5*(x(i,1)+x(i,2)); y(i,4)=0.5*(y(i,1)+y(i,2)); x(i,5)=0.5*(x(i,2)+x(i,3));
 y(i,5)=0.5*(y(i,2)+y(i,3)); x(i,6)=0.5*(x(i,3)+x(i,1)); y(i,6)=0.5*(y(i,3)+y(i,1));
end

for k=1:ne     % project boundary nodes onto a circle 
    rad=sqrt(x(k,5)^2+y(k,5)^2); fc=act/rad; x(k,5)=fc*x(k,5); y(k,5)=fc*y(k,5);
end
% refinement loop
if(ndiv > 0)
for i=1:ndiv
 ...
 for j=1:ne   % loop over current elements
  ...
  % these will become the "new" elements
   ...
   yn(nm,4)=0.5*(yn(nm,1)+yn(nm,2));   efln(nm,4)=0;
   xn(nm,5)=0.5*(xn(nm,2)+xn(nm,3));
   yn(nm,5)=0.5*(yn(nm,2)+yn(nm,3));   efln(nm,5)=0;
   xn(nm,6)=0.5*(xn(nm,3)+xn(nm,1));
   yn(nm,6)=0.5*(yn(nm,3)+yn(nm,1));   efln(nm,6)=0;
...
%---
 ...
 for k=1:ne  % relabel the new nodes and put them in the master list
  % project boundary nodes onto a circle of radius act
    for l=1:6
 ...
 Irepo=1;
 if(Irepo==1)
 for k=1:ne        % reposition the interior mid-nodes
    if(efl(k,4) == 0)
      x(k,4)=0.5*(x(k,1)+x(k,2)); y(k,4)=0.5*(y(k,1)+y(k,2));
    end
    if(efl(k,5) == 0)
      x(k,5)=0.5*(x(k,2)+x(k,3));  y(k,5)=0.5*(y(k,2)+y(k,3));
    end
    if(efl(k,6) == 0)
      x(k,6)=0.5*(x(k,3)+x(k,1)); y(k,6)=0.5*(y(k,3)+y(k,1));
    end
 end
 end
...
% plotting
 ...
 Iplot=1; Iplot=0;
 if(Iplot==1)
 figure(88);  hold on;  axis square;  xlabel('x','fontsize',15);
