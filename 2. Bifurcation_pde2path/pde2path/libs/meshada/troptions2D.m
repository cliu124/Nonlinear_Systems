function options=troptions2D()
% troptions2D: options for trullekul, 2D p2p-version 
% of what seems best for adaption based on p-norm with  z=p.u  
% see also z=stanzfu(p), eta=stanetafu(p,np); 
options.zfu=@stanzfu; 
options.etafu=@stanetafu; 
options.etafac=1e-5; 
options.setids=@setidssq; 
options.ppar=1000; 
% remainder: parameters from trullekruls gen_options
options.qualM=5; %mesh quality metric: vasilevski(1), fast orthogonal (2), 
% fast orthogonal vasilevski (3), steiner ellipse (4), steiner ellipse vasilevski (5)
options.qualP=0; %combines angles with quality in metric space (~=0), 
...
options.fastRM=2; %coarsening is performed on edges shorter than options.Lup only (1). 
...
options.rmnonconvec=2; %do not try to convexify non-convex sets during node-removal 
% or edge swapping (3D) (0), allow one concave point (1) or an infinite amount of them (2)
options.debug =0; %test inverted elements (1), wrong area (2 && area~=0), 
% correct internal value of element quality list (2)
...
options.innerit=2; %number of inner iterations
options.consMV=1; % accept smoothing regardless of whether it improves the minimum local element quality (0)
options.log_m_add=0; %add mesh metrics in log space (1)
options.nosparse=true; %use sparse function (speed increase)
options.ismatlab=true;
options.greedyCLR=2; %greedy colouring algorithm (1), largest ID first (0) or most neighboughs first 
% (fall back to ID in conflicts), greedy colouring with cromatic number reduction (2) 
% and the same with preference to most neigh boughs (3, extremely slow)
...
options.smpRFN=1; %only refine one edge in each element, perhaps using colouring (2)
...
options.RMnd3D=1; %prevent splitting of spheres instead of killing them after the fact (1,2) 
% and ditch spheres based on failed edge generation only (2,3)
options.consRFN=0; %only refine elements, when it improves all the local mesh quality (1) or 
% also when it only improves the worst local mesh quality (2)
