...
% floqap: Floquet multipliers and ind(u_H)  (a posteriori) 
% 
% [muv1, muv2, ind, h]=floqap(pd,varargin)
% 
% pd=(p2p-struct, or directory) 
% muv1,muv2=computed multipliers, ind=ind(u_H)
% mo=monodromy, h=plot-handle (to modify marker-size etc) 
%
% aux=varargin: wnr=fig-nr for plotting, nfloq=#of multipliers to compute
noa=nargin-1; k=1; 
if(ischar(pd)) 
...
t1=tic; 
[f,jac]=tomassempbc(p,y,p.hopf.T,lam); 
[AA,BB]=flmcoll(p,jac); ndim=nu^2*m; mo=speye(nu); 
if ndim<1e9; for k=1:m; fak=BB(:,:,k)\AA(:,:,k); mo=fak*mo; end; % building M 
else; for k=1:m; fak=BB{k}\AA{k}; mo=fak*mo; end; % problem was large, nece cell arrays
end
%tic; coeig=condeig(mo); max(coeig), toc
ok=0; 
while ~ok; 
