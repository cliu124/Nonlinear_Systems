...
for N=Nlist
    ...
    %[xy,tri,bndmesh,geomfunc] = mesh_Lfillet(N,1.5,0.1,options);
    ...
    %bndmesh = bndmesh_polygon(tri,xy,bndmesh,options); bndmesh.IDs(and(xy(bndmesh.edg(:,1),1) == 0,xy(bndmesh.edg(:,2),1) == 0)) = -1;
    tic;
