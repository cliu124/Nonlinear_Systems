...
par=[-0.1; 1; 0.1; -1; 1]; %  r  nu  mu   c3  c5
p=cGLinit(p,lx,nx,par,ndim); p=setfn(p,dir); p.sw.bifcheck=2; p.np, pause 
p.sw.verb=2; p.nc.mu1=0.5; p.plot.pstyle=1; p=cont(p,20);
...
AMG=0; p.sw.verb=2; % set AMG=1 if ilupack available (if no ilupack, bel might give memory problems!) 
...
%% Floquet a posteriori, this is probably slow!
