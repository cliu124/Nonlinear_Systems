...
%
% q=qswibra(p)                     - bare syntax 
% q=qswibra(p,newdir)               - set file.dir to newdir
% q=qswibra(p,newdir,aux)           - add args in aux 
% q=qswibra(dir,fname)              - load p from dir/fname
% q=qswibra(dir,fname,newdir)       - set file.dir to newdir
% q=qswibra(dir,fname,newdir,aux)   - use add.arg. from aux 
%
...
% isotol:  consider qbe soln isolated if det(Jac)>isotol/jmax^m (default 0.1) 
...
if verb>0 % plot the tangents 
  for i=1:min(nsol,9) 
    ...
    pw=ceil(i/3); set(figure(20+i),'Position',[scnw-pw*sz pheight sz sz]);
      if Xcont>0; p.up=[p.mat.qtau(1:p.nu,i);p.u(p.nu+1:end)]; end 
    up=p.mat.qtau(:,i); m1=min(up(1:p.nu)); m2=max(up(1:p.nu)); d=0.01/(1+m2-m1); 
