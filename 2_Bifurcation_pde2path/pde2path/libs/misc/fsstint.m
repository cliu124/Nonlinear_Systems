function [p,t1,ts,nc]=fsstint(p,t0,ts,dt,dtplot,nt,nc,N,del,varargin)
...
xg=getpte(p); x1=xg(1); x2=xg(end); L=x2-x1; dx=L/N; np=p.np; t=t0; 
...
if prep==1; % first call, starting from the REAL ground state
   u1=interp1(xg,p.u(1:np),x); u=(1+del)*u1+0i*u1; p.prep=0; 
   save([p.file.dir '/0.mat'], 'x','u','t'); 
else  u=p.uc(1:N); 
end 
par=p.u(p.nu+1:end); lam=par(1); s=par(2); sig=par(3); ga=par(4); 
V=pot(x,s)+lam; % add lam to V, i.e., DNS in co-rot.frame 
phi=u; figure(3); plot(x,real(u),x,imag(u));  
