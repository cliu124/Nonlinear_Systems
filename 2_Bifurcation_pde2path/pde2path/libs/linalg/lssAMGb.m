function [x,p]=lssAMGb(A,b,p)
% lssAMGb: Uses ilupack (www.icm.tu-bs.de/~bolle/ilupack/) for solving Ax=b. 
% needs mexing of ilupack first
% 
% [x,p]=lssAMGb(A,b,p)
% see also: lss, lssbel, blssbel, belpi, bel
nb=norm(b(:),inf); if nb==0; x=b; return; end   % rhs=0 
if ~isfield(p,'amgopt'); p.amgopt=AMGinit(A,[]); end % very first call of lssAMG 
...
if (~isfield(p.mat,'prec') || size(A,1)~=s) % if prec does not ex. or has wrong size 
  ...
  t1=tic; [p.mat.prec,p.amgopt]=AMGfactor(A,p.amgopt); t1=toc(t1); newprec=1; % compute prec.
  if p.sw.verb>1; fprintf(' in %g seconds\n', t1); end 
  s=p.mat.prec(1).n; 
end
t1=tic; [x,p.amgopt]=AMGsolver(A,p.mat.prec,p.amgopt,b); t1=toc(t1); 
...
while (used_it>p.amgopt.maxit-1 && newprec==0) % no convergence 
    if newprec==0 % if preconditioner was not updated, do now and try again 
      if p.sw.verb>1; fprintf('\n'); disp('AMGsolver did not converge...new PREC'); end
      p.mat.prec=AMGdelete(p.mat.prec); p.mat=rmfield(p.mat,'prec'); % clean mem 
      p.amgopt=AMGinit(A,[]); p.amgopt=ilupparam(p,p.amgopt); 
      t1=tic; [p.mat.prec,p.amgopt]=AMGfactor(A,p.amgopt); t1=toc(t1); newprec=1; 
      if p.sw.verb>1; fprintf(' in %g seconds\n', t1); end 
      t1=tic; [x,p.amgopt]=AMGsolver(A,p.mat.prec,p.amgopt,b); t1=toc(t1); 
      used_it=max(p.amgopt.niter); 
      if p.sw.verb>2; fprintf('AMGsolver in %g seconds, niter=%i \n', t1, used_it); end       
