function [tri,xy,Nmetric,bndmesh,triQ]=huadapt_mesh(sw,tri,xy,Nmetric,bndmesh,geomfunc,options)
...
% hence, decimal: 
%  sw       1      2      3      4       5    6     7     8     9  ... 16
% action: move   refine  m,r  coarsen   c,m  c,r  c,r,m  swap  s,m    all  
% 
% hence sw=16: orginal adapt_mesh behaviour
...
[tri,xy,bndmesh,Nmetric,triQ,bks] =pragadapt(tri,xy,Nmetric,bndmesh,triQ,bks,geomfunc,options,sw);
else   
...
function [tri,xy,bndmesh,Nmetric,triQ,bks] =pragadapt(tri,xy,Nmetric,bndmesh,triQ,bks,geomfunc,options,sw); 	
  ...
  for i=1:options.innerit;  if options.verbose == 1;  disp_qual(triQ,tri,xy,i); end;
	...
	if ismember(sw,[2,3,6,7]); %REFINEMENT
	...
	if ismember(sw,[1,3,5,7]); %SMOOTHING
