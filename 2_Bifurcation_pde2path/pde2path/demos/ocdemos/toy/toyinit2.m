function p=toyinit(~,nx,nt,par) % init file for the CPS
%p=loadp('hopf_p','pt24'); % loading hopf structure for basic parameters %
% HdW 26.09.
p=[]; % HdW 26.09.
lx=1; p=stanparam(p); screenlayout(p); % set standard parameters and screenlayout 
p=hostanparam(p); % HdW 26.09.
% --- HdW ---
p.hopf.tom.Stats='on';
p.hopf.tom.Stats_step='on';
p.hopf.tom.Monitor=3;
p.hopf.tom.Order=2;
p.hopf.tom.AbsTol=1.0000e-04;
p.hopf.tom.Itnlmax=20;
p.hopf.tom.Itlinmax=20;
p.hopf.tom.FJacobian=@hojac;
p.hopf.tom.BCJacobian=@hobcjac;
p.hopf.tom.lu=0;
p.hopf.tom.vsw=0;
% --- HdW ---
...
p.sw.sfem=-1; p.np=1;%pde.grid.nPoints;
...
p.mat.M=eye(4,4); % replace mass and "lapacian"-Matrix
p.mat.K=0*eye(4,4);
p.hopf.tom.M=p.mat.M;
...
%% HdW 26.11. aimed end-point on CSS is at position "end-1"
y=p.hopf.y;
yshift=circshift(y(:,1:end-1),-1,2);
