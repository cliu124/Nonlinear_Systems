function zdia=lvdiagnT(p,wnr,sw) % diagnostics for lv-bd-control 
% sw=1: J,  2: k_1,2,  3: v_1,2,   4: lam_1,lam_2,  5: h1,h2
s1=p.oc.s1; sol=p.cp; set(0,'defaultlinelinewidth',2)
x=sol.par(1)*sol.t; y=sol.u; sl=length(x); n=s1.np; 
...
for i=1:sl
  switch sw
      ...
      case 2; k=lvcon(s1,[y(:,i); par]); zdia(1,i)=k(1); zdia(2,i)=k(2); 
      case 3; zdia(1,i)=y(1,i); zdia(2,i)=y(n+1,i); 
      case 4; zdia(1,i)=y(2*n+1,i); zdia(2,i)=y(3*n+1,i);    
      case 5; k=lvcon(s1,[y(:,i); par]); v1=y(1,i); v2=y(n+1,i); 
...
rho=s1.u(s1.nu+8); jp=jcaiT(s1,sol,rho)+disjcaT(s1,sol,rho,0); % jca of path
j0=jca(s1,p.u0)/rho; j1=jca(s1,s1.u)/rho; 
tit=['Eq to c1/pt0, J=' mat2str(real(jp),4)]; 
fprintf('Jp=%g, J1=%g\n',jp,j1); 
t0=min(x); t1=max(x); 
