...
for j=1:ngen  % loop over refinements 
  ...
  par=u(p.nu+1:end); tau=p.tau(1:p.nu); tauf=p.mat.fill*tau; lamd=p.tau(p.nu+1:end);     
  [p,idx]=p.fuha.e2rs(p,p.u); % select elements to refine 
  fprintf('refining %i elements\n',length(idx)); p.pdeo.grid.refineMesh(idx);   
  p=two2lob(p); %p=setfemops(p); 
  %p.hofem, pause 
  npo=p.pdeo.grid.p; np=p.pdeo.grid.nPoints; p.sol.xi=1/np; 
