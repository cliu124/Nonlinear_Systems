...
p2pglob.faceal=1; p2pglob.cut=1; p2pglob.tsw=0; p2pglob.showbd=2; 
...
p=nodinit(p,lx,ly,nx,ny,par,sym); p=setfn(p,'N'); huclean(p); pplot(p); 
p.file.smod=1; p.sw.bifloc=0; p.sf=1e2;
p.sw.jac=1; p=cont(p,30); %p.sw.jac=0; p=cont(p,30);
...
%% 2nd BP, double 
p1=qswibra('N','bpt2',aux); p=gentau(p1,[0.5 0.5],'N2'); p.sol.ds=0.125; p.nc.tol=1e-3;  
p.sw.bifcheck=0; p=cont(p,2); p.sw.jac=1; p.nc.nq=2; p.nc.ilam=[3 1 6]; p.fuha.qf=@qfArot; 
