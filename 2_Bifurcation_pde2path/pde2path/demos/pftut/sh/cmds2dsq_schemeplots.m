...
lx=pi; nx=round(10*lx); ly=lx; ndim=2; lam=-0.05; nu=0; par=[lam; nu];  
p=shinit(p,nx,lx,ly,ndim,par); p=setfn(p,'2Ds/0'); huclean(p); p=cont(p,10); 
%% nu=0, hence, sp (spots) supercrit&unstable, st (stripes) supercrit&stable 
p0=cswibra('2Ds/0','bpt1'); % cswibra, then reset some parameters
p0.nc.dsmax=0.05; p0.nc.dsmin=0.01; p0.nc.lammax=1; p0.sol.ds=0.05; 
p0.sw.foldcheck=0; p0.sw.bifcheck=0; 
%% inspecting the results of cswibra yields tau1=spots, tau3=stripes, hence 
% select these and call cont. 
p=seltau(p0,1,'2Ds/1-1aSp'); p=cont(p,10); 
p=seltau(p0,3,'2Ds/1-1aSt'); p=cont(p,10);
...
p0.u(p0.nu+2)=0.7;  p0=cswibra(p0,aux); p0.sol.ds=0.01; p0.nc.dsmax=0.02; pause 
p=seltau(p0,1,'2D/1-1bSp'); p=cont(p,10); p=seltau(p0,3,'2D/1-1bSt'); p=cont(p,10);
...
p0.u(p0.nu+2)=0.8; p=seltau(p0,1,'2D/1-1cSp'); p=cont(p,10); p=seltau(p0,3,'2D/1-1cSt'); p=cont(p,10);
%% nu=1; a_1>0, a_2<0, a1<-a2, both unstab
p0.u(p0.nu+2)=1; p=seltau(p0,1,'2D/1-1dSp'); p=cont(p,10); p=seltau(p0,3,'2D/1-1dSt'); p=cont(p,10);
%% plot BD, nu=0
fnr=4; cmp=0; figure(fnr); clf; plotbra('2Ds/0','bpt1',fnr,cmp,'cl','k');
plotbra('2Ds/1-1aSp','pt10',fnr,cmp,'cl','b'); 
plotbra('2Ds/1-1aSt','pt10',fnr,cmp,'cl','r'); 
title('\nu=0'); xlabel('\lambda'), ylabel('max(|u|)'); box on;
axis([-0.05 0.1 0 1.5]); 
%% plot BD, nu=0.7
fnr=4; figure(fnr); clf; plotbra('2Ds/0','bpt1',fnr,cmp,'cl','k');
plotbra('2D/1-1bSp','pt10',fnr,cmp,'cl','b'); plotbra('2D/1-1bSt','pt10',fnr,cmp,'cl','r'); 
title('\nu=0.7'); xlabel('\lambda'), ylabel('max(|u|)'); box on; axis([-0.005 0.01 0 1]); 
%% plot BD, nu=0.8 
fnr=4; figure(fnr); clf; plotbra('2Ds/0','bpt1',fnr,cmp,'cl','k');
plotbra('2D/1-1cSp','pt10',fnr,cmp,'cl','b'); plotbra('2D/1-1cSt','pt10',fnr,cmp,'cl','r'); 
title('\nu=0.8'); xlabel('\lambda'), ylabel('max(|u|)'); box on;  axis([-0.005 0.007 0 1]); 
%% plot BD nu=1
fnr=4; figure(fnr); clf; plotbra('2Ds/0','bpt1',fnr,4,'cl','k');
plotbra('2D/1-1dSp','pt10',fnr,4,'cl','b'); plotbra('2D/1-1dSt','pt10',fnr,4,'cl','r'); 
title('\nu=1'); xlabel('\lambda'), ylabel('max(|u|)'); box on; axis([-0.015 0.001 0 0.3]); 
