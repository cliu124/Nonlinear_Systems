...
p=shinit(p,nx,lx,ly,ndim,par,lz,sw); p=setfn(p,'SC/0'); plotsol(p,1,1,3); p.np, pause 
...
p=seltau(p0,1,'SC/l'); pause, p=pmcont(p,20); % lamella
p=seltau(p0,4,'SC/t'); pause, p=pmcont(p,20); % tube
p=seltau(p0,11,'SC/r'); pause, p=pmcont(p,20); % rhomb
%% plot BD and solns 
fnr=3; cmp=4; figure(fnr); clf; plotbra('SC/t','pt20',fnr,cmp,'cl','b','lab',10);
plotbra('SC/l','pt20',fnr,cmp,'cl','r','lab',15); plotbra('SC/r','pt20',fnr,cmp,'cl','m','lab',15);
title('\nu=0'); xlabel('\lambda'), ylabel('max(|u|)'); box on; 
plotsol('SC/t','pt20'); pause; plotsol('SC/l','pt20'); pause; plotsol('SC/r','pt20'); 
...
p=seltau(p0,1,'SC/l2'); p=pmcont(p,15); % lamella
p=seltau(p0,4,'SC/t2'); p=pmcont(p,15); % tube
p=seltau(p0,11,'SC/r2'); p.nc.tol=1e-6; p=pmcont(p,5); p.nc.tol=1e-8; p=pmcont(p,15); % rhomb 
%% BD and solution plot 
fnr=3; cmp=4; figure(fnr); clf; 
plotbra('SC/l2','pt15',fnr,cmp,'cl','r');
plotbra('SC/t2','pt10',fnr,cmp,'cl','b'); 
plotbra('SC/r2','pt10',fnr,cmp,'cl','m'); 
title('\nu=1.2'); xlabel('\lambda'), ylabel('max(|u|)'); axis([-0.08 0.15 0 1.5]); box on; 
plotsol('SC/r2','pt10',1,1,2); view(-15,10); 
%% nu=0.75: rhombs supercrit and stable at bif, but continue as tubes! 
huclean(p); p0.u(p0.nu+2)=0.75; 
%p=seltau(p0,1,'SC/l3');  p=pmcont(p,10); % lamella
%p=seltau(p0,5,'SC/t3'); p=pmcont(p,10); % tube 
p=seltau(p0,7,'SC/r3'); pause, p.nc.tol=1e-8; p.pm.resfac=1e-6; p.file.smod=5; p=pmcont(p,10); % rhomb 
%% nu=0.75 plots
fnr=3; cmp=4; figure(fnr); clf; plotbra('SC/l3','pt5',fnr,cmp,'cl','m');
plotbra('SC/t3','pt10',fnr,cmp,'cl','b'); plotbra('SC/r3','pt10',fnr,cmp,'cl','r');
title('\nu=0.75'); xlabel('\lambda'), ylabel('max(|u|)'); box on; 
plotsol('SC/r3','pt5',1,1,2); 