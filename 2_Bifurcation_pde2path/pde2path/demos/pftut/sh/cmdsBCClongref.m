...
q=loadp('b2z','pt10'); plotsol(q); % load old (coarse point) 
%% init (slightly) finer mesh 
p=[]; lx=sqrt(2)*pi/2; ly=lx; lz=8*lx; ndim=3; lam=-0.001; nu=1.5; par=[lam; nu];  
nx=7; sw.sym=1; sw.ref=0;  dir='b2zref'; % np=11360
...
plotbra('b2z','pt200',fnr,cmp,'cl',p2pc('r1'),'lab',10,'fp',10,'lab',200); 
plotbra('b2zref','pt200',fnr,cmp,'cl','b','lab',200); 
plotbra('b2ztr','pt200',fnr,cmp,'cl','m','fp',11,'lab',[12 100 200]); 
xlabel('\lambda'); ylabel('||u||'); 
%% 3D trulle 
p=loadp('b2z','pt10','b2ztr'); p.plot.EdgeColor='k'; p.plot.pstyle=3; 
...
stansavefu(p);
%%
p=loadp('b2ztr','pt11'); p.sol.ds=0.02; p.nc.dsmax=0.02;  
p.nc.amod=10; p.nc.ngen=2; p=pmcont(p,40); 
%%
v=[140,30]; 
p=loadp('b2zref','pt10'); p.plot.EdgeColor='k'; p.plot.pstyle=3; 
p.plot.cm='cool'; p.plot.shsw=0; plotsol(p); colorbar off; view(v); pause 
plotsol('b2ztr','pt12'); colorbar off; view(v); pause; 
plotsol('b2ztr','pt200');view(v); colorbar off;
%%
plotsol('b2ztr','pt100'); colorbar off;
