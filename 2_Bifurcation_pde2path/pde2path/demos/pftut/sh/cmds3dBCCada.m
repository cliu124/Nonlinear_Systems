...
%% SH on BCC, illustration of mesh-ada with trullekrul, small domain 
p=[]; lx=sqrt(2)*pi; ly=lx; lz=lx; ndim=3; lam=-0.001; nu=1; par=[lam; nu];  
nx=6; sw.sym=1; sw.ref=1; dir='3D/BCC1b'; % np=3161
...
p=seltau(p0,1,'3D/BCCb2',2); p.sol.ds=-0.02; p=pmcont(p,40); % hot BCC work well  
%% use gentau for tubes; these also loose symmetry on this coarse mesh 
p=gentau(p0,[0 1 0],'3D/BCCt2'); p=pmcont(p,10); 
%% refine BCCs, 
p=loadp('3D/BCCb2','pt5','3D/BCCb2r'); p=resetc(p); p.plot.shsw=0; 
...
%% extra coarsening (test it!) 
trops=p.trop; p.trop=p.trcop; p=oomeshada(p,'ngen',1); p.trop=trops; stansavefu(p); 
%% cont with adaption each 3rd step 
p=loadp('3D/BCCb2r','pt0','3D/BCCada'); p.sw.ips=2; 
p.nc.amod=3; p.trop.innerit=5; p.trcop.sw=13; p.trcop.crmax=2; 
p=cont(p,10); 
%% plot BD 
fnr=3; cmp=3; figure(fnr); clf; plotbra('3D/BCC1',fnr,cmp,'cl','k','lp',3); 
plotbra('3D/BCCb2','pt10',fnr,cmp,'cl',p2pc('r1'),'lab',5);
plotbra('3D/BCCada','pt10',fnr,cmp,'cl',p2pc('r2'),'lab',20); 
xlabel('\lambda'), ylabel('||u||'); box on; 
%% solns plots 
v=[-50,25]; 
plotsol('3D/BCCb2','pt15'); view(v); xlabel(''); ylabel(''); zlabel(''); pause; 
plotsol('3D/BCCada','pt10'); view(v); xlabel(''); ylabel(''); zlabel(''); 
%% move soln to an alternative (uniform) mesh, first load two solns (meshes) 
p=loadp('3D/BCCb2','pt5','3D/BCCb2fix'); q=loadp('3D/BCCa','pt5','3D/BCCa2'); 
%% call pa2pb 
