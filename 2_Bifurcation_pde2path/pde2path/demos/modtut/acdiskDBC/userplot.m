function userplot(p,wnr) % for AC2D with Fourier-cheb, and DBCs from bdfu  
% the origin (r=0) is not in the mesh, but plots are 'filled in' if p.ipz=1 
figure(wnr); clf; gbd=bdfu(p,p.u);
u=p.u(1:p.np); na=p.na; nr=p.nr; th=p.th; 
[rr,tt]=meshgrid(p.lx*p.r,[0;th]); [xx,yy]=pol2cart(tt,rr); 
u=reshape(u,na, nr-1); u=[gbd u]; % fill in bdry 
u=u([na 1:na],:); % copy last row to front to fill circle 
