function p=acinit(lx,ly,nx,par,varargin) % ac2D 
sw.sym=0; if nargin>5; sw=varargin{1}; end % switch for meshing symmetry 
p=stanparam; screenlayout(p); p.nc.neq=1; p.sw.sfem=-1; p.fuha.e2rs=@e2rs; 
pde=stanpdeo2D(lx,ly,2*lx/nx,sw); % 2D rectangle pde object, h as argument
