...
p.sol.ds=-0.1; p.fuha.spjac=@spjac; p.usrlam=0.3; mclf(2); p=cont(p,10); 
%% c6 - plot lam-pos over c
figure(3); clf; plotbra('bpcon', 'pt10');
%% c7 - exit BP cont 
p=bpcontexit('bpcon','pt10','trb'); % switch to continuation in lambda again
...
%% BP cont with num spjac
p=bpcontini('tr','bpt1',1,'du'); % init branch point cont. in par(1) (c)
p.sw.bifcheck=0; p.plot.bpcmp=2; % plot lam over c now
p.sol.ds=-0.1; p.fuha.spjac=@bpjac; p.usrlam=0.3; 
p.sw.spjac=0; p.sw.jac=1; % spjac=0 OK, but jac=0 not working yet 
