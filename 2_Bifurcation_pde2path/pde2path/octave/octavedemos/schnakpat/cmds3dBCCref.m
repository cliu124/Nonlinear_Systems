...
nx=6; sw.sym=1; sw.ref=1; dir='BCC2'; % np=6006, still OK
...
p.nc.neig=10; p.sw.verb=2; p.nc.tol=1e-6; 
...
p.trop=op;   % put options in p 
 p.nc.tol=1e-8; 
op.npb=6000; op.sw=5; op.Llow=100; op.innerit=3;  p.trcop=op; 
p.sw.trul=1; stansavefu(p);
%%
p=oomeshada(p,'ngen',1); stansavefu(p); 
%%
plotsol(p,6,1,3); colormap cool; 
plotsol(p,7,1,3); 
%% extra coarsening 
