...
function unitSquare(obj,hmax)
    % obj.unitsquare[hmax]), (c) 2013 Uwe Prüfert
...
function doubleT(obj,scaleFactor)
    ...
    % doubleT()
    % doubleT(scale)
    % Called with no argument, the base has lenght one and the
...
function unitCircle(obj,hmax)
    % obj.unitcircle(), obj.unitcircle(hmax), (c) 2013 Uwe Prüfert
    switch nargin
        case 1
            obj.circle(1,0.2);      
        case 2
...
function freeGeometry(obj,varargin)
    ...
    %(c) 2013 by Uwe Prüfert
    % Since DelaunayTri disapears in future versions, we must
    % handle ode/new release things...
    vers=version('-release');

    if nargin==1 % no constraint given?
        obj.wrongNumberInputs.throw;
    elseif nargin==2  % only one outer constraint
        % prepare data
        X=varargin{1};
        [xx,xy]=size(X);
        if min(xx,xy)~=2
        ...
        [nx,ny]=size(xm); xm=reshape(xm,1,nx*ny); ym=reshape(ym,1,nx*ny);
        % the constraint is the index of boundary elements
        C= [1:np;2:np,1]'; 
        X=[X';[xm;ym]'];                    
        if str2double(vers(1:4))<2013 % old code                  
            dt=DelaunayTri(X,C); io=dt.inOutStatus; p=dt.X';
            t=dt.Triangulation';
        else
            dt=delaunayTriangulation(X,C); p=dt.Points';  
             t=dt.ConnectivityList';  io=dt.isInterior;                        
        end

        % Detect all triangles inside the object, Select points from inside triangles
        p=p(:,unique(t(:,io))'); t=t(:,io);
% New triangulation only with inside points. We need this iteration because of the renumbering
% within the triangle-point relation. We need here the  old outer point removed field.
        if str2double(vers(1:4))<2013 % old code
            dt=DelaunayTri(p',C); io=dt.inOutStatus; t=dt.Triangulation';
        else
             dt=delaunayTriangulation(p',C); t=dt.ConnectivityList';
             io=dt.isInterior;                        
        end                
        obj.p=p; obj.e=C'; obj.t=t(:,io); obj.t(4,:)=ones(1,size(obj.t,2)); 
        % the parameter s runs from 0 to 1 on the boundary
        sds=sum(ds); obj.e(3,1)=0; obj.e(4,1)=ds(1)/sds; obj.e(5,1)=1;
    ...
    else  % a number of (interior) boundaries.
        % prepare data
        % outer "constraint"
        ...
        for k=1:length(varargin)
            ...
            if xy==2 % it should be a matrix x;y coordinate
                X=X';
            end
            ds= sqrt(sum([(X(:,2:end)-X(:,1:end-1)).^2 (X(:,1)-X(:,end)).^2]));
            dsmean(k)=mean(ds);
        ...
        dsmin=min(dsmean);
        XX=[]; XC=[]; NBDS=[]; % the 5-th row in obj.e=the number of boundary segment 
        XDS=[];  XE=[];
        for k=1:length(varargin)
            X=varargin{k};
            [xx,xy]=size(X);
            ...
            NBDS=[NBDS,k*ones(1,np)]; XC=[XC C]; XX=[XX X];  XDS=[XDS ds];
            sds=sum(ds);
            e3=0; e4=ds(1)/sds;                      
            for k1=2:np                             
                e3(k1)=e4(k1-1); e4(k1)=e4(k1-1) + ds(k1) /sds;                                                       
            end                        
            XE=[XE [e3;e4]];
        end
        h=mean(XDS);
        ...
        if str2double(vers(1:4))<2013  % old code
            fprintf('old'); dt=DelaunayTri(X,XC'); io=dt.inOutStatus;
...
% Detect all triangles inside the object, select all points from inside-triangles
        p=p(:,unique(t(:,io))'); t=t(:,io);

        % New triangulation only with inside points
        if str2double(vers(1:4))<2013  % olde code                   
            dt=DelaunayTri(p',XC'); io=dt.inOutStatus; p=dt.X';
            t=dt.Triangulation';
        else
             dt=delaunayTriangulation(p',XC');  p=dt.Points';  
             t=dt.ConnectivityList'; io=dt.isInterior;                        
        end
        obj.p=p; obj.e=XC; obj.e(3:4,:)=XE;
% We have the same number of boundary segments as arguments.
        obj.e(5,:)=NBDS;
...
function holeInPlane(obj)
    ...
    [p,e,t]=distmesh2dOO(fd,fh,0.1,[-1,-1;1,1],pfix); 
...
obj=setidssq(obj); obj.t(4,:)=ones(1,size(obj.t,2)); 
end 

function squarechebx(obj,varargin) % cheb in x  *********************************
gotnx=0; 
switch nargin
    case 6 % corners and hmax
        xmin=varargin{1}; xmax=varargin{2}; ymin=varargin{3}; ymax=varargin{4}; h=varargin{5};           
...
if ~gotnx; nx=max(2,round((xmax-xmin)/h)+1); ny=max(2,round((ymax-ymin)/h)+1); end     
x=cos(pi*(0:nx-1)/(nx-1))'; x=flip(x); xmesh=xmax*x'; 
ymesh=linspace(ymin,ymax,ny); [X,Y]=meshgrid(xmesh,ymesh);            
P=[reshape(X,1,nx*ny); reshape(Y,1,nx*ny)];
dt=DelaunayTri(P'); obj.p=dt.X'; obj.e=dt.freeBoundary'; obj.t=dt.Triangulation'; 
enx=linspace(0,1,nx); eny=linspace(0,1,ny); obj.e(3,1:nx-1)=enx(1:end-1); 
obj.e(3,nx:nx+ny-2)= eny(1:end-1); obj.e(3,nx+ny-1:nx+ny+nx-3)=fliplr(enx(1:end-1));
...
function squarechebxy(obj,varargin) % Cheb in x and y *******************************
gotnx=0; 
switch nargin
    case 6 % corners and hmax
        xmin=varargin{1}; xmax=varargin{2}; ymin=varargin{3}; ymax=varargin{4}; h=varargin{5};           
...
if ~gotnx; nx=max(2,round((xmax-xmin)/h)+1); ny=max(2,round((ymax-ymin)/h)+1); end     
x=cos(pi*(0:nx-1)/(nx-1))'; x=flip(x); xmesh=xmax*x'; 
y=cos(pi*(0:ny-1)/(ny-1))'; y=flip(y); ymesh=ymax*y'; 
...
function ellipse(obj)
    ...
    [p,e,t]=distmesh2dOO(fd,@huniform,0.2,[-2,-1;2,1],[]);
    obj.p=p'; obj.e=e'; obj.t=t';
end
...
function plot(obj,varargin)   
    ...
    if (nargin == 1) || (mod(nargin-1,2) == 0)                     
        ...
        if length(z) == obj.nPoints       
            ...
            set(gca,'Box','on');  view(3); %   grid(ax,'on'); colorbar;
...
function [cval,aval,fval]=aCoefficientsMpt(obj,c,a,f) % HU: needs cleanup! 
    ...
    switch class(c)
        ...
        case 'double'
            % four cases: (i) scalar (ii) 2 x 2 matrix 
            %  (iii) vector length np, (iv) vector length nt
            [rows,cols]=size(c); %rows,cols, pause 
        ...
        case 'char'
            ...
            midp=obj.midpts; x=midp(1,:); y=midp(2,:); 
            try; cval=eval(c); catch ME; throw(ME); end
            [rows,cols]=size(cval);
            switch max(rows,cols)
            ...
            case 'cell'
        % must be a 2*2 cell array which refers to the entries of a 2*2
        % diffusion matrix. Every Entry must be double or a single char symbolizing
        % the part of the coefficent function like in the case 'char'.
        [rows,cols] = size(c); %rows, cols 
        if (rows ~=2) || (cols ~=2); throw(obj.wrongFormat);  end
        cval = zeros(4,obj.nElements);  cvalcell = cell(2,2);
    ...
    switch class(a)
        case 'function_handle'
            midp=obj.midpts; x=midp(1,:); y=midp(2,:); aval=feval(a,x,y);
            [rows,cols]=size(aval);
            if max(rows,cols)==obj.nPoints
                aval= obj.point2Center(aval); aval=aval(:)';
            end  
        case 'double'
            [rows,cols]=size(a);
            switch max(rows,cols)
                case 1; aval=a(ones(1,obj.nElements));            
                case obj.nPoints
                    aval=obj.point2Center(a); aval=aval(:)';    
                case obj.nElements % the good one, nothing to do
                    aval=a(:)';
                otherwise
                    obj.wrongFormat.throwAsCaller
        ...
        case 'char'         
            midp=obj.midpts; x=midp(1,:); y=midp(2,:);
            try; aval=eval(a); catch ME; ME.throwAsCaller; end
            [rows,cols]=size(aval);
            switch max(rows,cols)
                case 1; aval=aval(ones(1,obj.nElements));             
                case obj.nElements; % work already done 
                otherwise; finiteElements.wrongFormat.throwAsCaller;
            end
        case 'cell'
            obj.wrongClass.throwAsCaller;
        otherwise
    ...
    switch class(f)
        case 'function_handle'
            midp=obj.midpts; x=midp(1,:); y=midp(2,:); fval=feval(f,x,y);
            [rows,cols]=size(fval);
            if max(rows,cols)==obj.nPoints; fval= obj.point2Center(fval); end  
        case 'double'  
            [rows,cols]=size(f);          
            switch max(rows,cols)
                case 1; fval=f(ones(1,obj.nElements));
                case obj.nPoints;  fval=obj.point2Center(f);  
                case obj.nElements;   fval=f; % the good one, nothing to do                     
                otherwise; obj.wrongFormat.throw;
            end
        case 'char'        
            midp=obj.midpts; x=midp(1,:); y=midp(2,:);
            try; fval=eval(f); catch ME; throw(ME); end
            [rows,cols]=size(fval);
            switch max(rows,cols)
                case 1; fval=fval(ones(1,obj.nElements));
                case obj.nElements % work already done
...
function pts2dom(obj,P) % point-cloud to PTE structure; all boundary edges get identifier 1, HU  
 dt=delaunayTriangulation(P'); obj.p=dt.Points'; 
 obj.e=dt.freeBoundary'; obj.t=dt.ConnectivityList'; 
 obj.t(5,:)=ones(1,size(obj.t,2));  setids1(obj); 
end   
...
function annulus(obj,r1,r2,h)  % HU
    ...
    [p,e,t]=distmesh2dOO(fd,fh,h,[-1,-1;1,1],pfix); 
    obj.p=p'; obj.t=t'; obj.e=e';
end

function freeGeoPts(obj,X,P,bdsi) % mod of grid2D.freeGeometry, with inner point list 
% X=bd-points, P=inner grid points, bdsi= splitting points for bd-segment identific. 
 xy=size(X,2);  if xy==2; X=X';  end
 ds= sqrt(sum([(X(:,2:end)-X(:,1:end-1)).^2 (X(:,1)-X(:,end)).^2]));
 if ds(end) < 1e-6 % we assume now that X ist a closed curve
  X(:,end)=[]; ds(end)=[];
 end
 np=length(ds); h=mean(ds);   % update number of points
 xm=P(1,:); ym=P(2,:); % from here as in org freeGeometry 
 [nx,ny]=size(xm); xm=reshape(xm,1,nx*ny); ym=reshape(ym,1,nx*ny);
 % the constraint is the index of boundary elements
 C= [1:np;2:np,1]';
 X=[X';[xm;ym]'];     
 dt=delaunayTriangulation(X,C);
 p=dt.Points';  t=dt.ConnectivityList'; io=dt.isInterior;                        
 % Detect all triangles inside the object, select all points from inside triangles
 p=p(:,unique(t(:,io))');  t=t(:,io);   
 dt=delaunayTriangulation(p',C);            
 t=dt.ConnectivityList'; io=dt.isInterior;      
 obj.p=p;  obj.e=C'; obj.t=t(:,io);
 obj.t(4,:)=ones(1,size(obj.t,2));  
 % the parameter s runs from 0 to 1 on the boundary
 sds=sum(ds);  obj.e(3,1)=0;
 obj.e(4,1)=ds(1)/sds;   obj.e(5,1)=1;
 % parametrize the boundary and assign identifiers
 nbd=length(bdsi); 
 for k=2:np
    obj.e(3,k)=obj.e(4,k-1);   
     obj.e(4,k)=obj.e(4,k-1) + ds(k) /sds;
     obj.e(5,k)=nbd+1;
     for i=1:nbd
        if k<bdsi(i); obj.e(5,k)=i; break; end % bd segment number
      end
 end
end
...
patch('faces',obj.t(1:3,:)','vertices',[x(:),y(:),z(:)],...
      'facevertexcdata',z0(:),'facecolor','interp', ...
        'edgecolor','k', 'parent',ax,varargin{2:end}); 
 set(gca,'Box','on'); colormap cool; view([-30 40]); set(gca,'FontSize',14); 
 axis(ax,'tight'); axis(ax,'image'); 
end
...
function coneplot2(obj,z0,a,e,varargin) % cone plot, HU                         
ax=axescheck(varargin{:}); ax=newplot(ax);  
x=obj.p(1,:); y=obj.p(2,:); z=-a*sqrt(x.^2./e^2+y.^2); 
patch('faces',obj.t(1:3,:)','vertices',[x(:),y(:),z(:)],...
      'facevertexcdata',z0(:),'facecolor','interp', ...
        'edgecolor','none',  'parent',ax,varargin{2:end}); 
 set(gca,'Box','on'); colormap cool; view([-30 40]);   set(gca,'FontSize',14); 
...
function coneplot3(obj,z0,h,e,varargin) % cone plot, height=h, ellipt=e                     
ax=axescheck(varargin{:}); ax=newplot(ax);  
x=obj.p(1,:); y=obj.p(2,:); z=h*(1-sqrt(x.^2+y.^2)); 
patch('faces',obj.t(1:3,:)','vertices',[e*x(:),y(:),z(:)],...
      'facevertexcdata',z0(:),'facecolor','interp', ...
        'edgecolor','k',  'parent',ax,varargin{2:end}); 
 set(gca,'Box','on'); colormap cool; view([-30 40]);   set(gca,'FontSize',14); 
 axis(ax,'tight'); axis(ax,'image'); 
